...
# Label genes
cir_gene_symbols <- AnnotationDbi::mapIds(org.Dm.eg.db,
                                      keys = rownames(mat_cg),
                                      column = "SYMBOL",
                                      keytype = "ENSEMBL",
                                      multiVals = "first")
rownames(mat_cg) <- ifelse(!is.na(cir_gene_symbols), cir_gene_symbols, rownames(mat_cg))
...
cg_nc_df <- as.data.frame(cg_normalized_counts)
# Save to a CSV file
write.csv(cg_nc_df, file = "normalized_counts_circadian_genes.csv")

# --- 2. Neuropeptides ---
neuropeptides <- c("FBgn0032840", "FBgn0027109")

# Normalized counts already is already set as data frame, therefore extract normalized counts for hunger associated genes
np_normalized_counts <- normalized_counts[neuropeptides, ]
# Save as csv
write.csv(as.data.frame(np_normalized_counts), "np_normalized_counts.csv")

# --- 3. Other Genes of Interest ---

# Extract normalized counts for takeout
gene_counts_to <- plotCounts(dds, gene="FBgn0039298", intgroup=c("Genotype", "Timepoint"), returnData=TRUE)
write.csv(gene_counts_to, file = "takeout_normalized_counts.csv", row.names = TRUE)

# PDF counts
gene_counts_pdf <- plotCounts(dds, gene="FBgn0023178", intgroup=c("Genotype", "Timepoint"), returnData=TRUE)
write.csv(gene_counts_pdf, file = "pdf_normalized_counts.csv", row.names = TRUE)

# ITP counts
gene_counts_itp <- plotCounts(dds, gene="FBgn0035023", intgroup=c("Genotype", "Timepoint"), returnData=TRUE)
write.csv(gene_counts_itp, file = "itp_normalized_counts.csv", row.names = TRUE)
